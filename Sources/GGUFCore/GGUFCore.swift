...
public enum GGUFValueType: UInt32, CustomStringConvertible, Sendable {
    case uint8 = 0
    case int8 = 1
    case uint16 = 2
    case int16 = 3
...
public enum GGUFMetadataValue: CustomStringConvertible {
    case scalar(String)
    case string(String)
    case array(type: GGUFValueType, count: UInt64, values: [String])

    public var description: String {
        switch self {
        case .scalar(let value), .string(let value):
            return value
        case .array(let type, let count, let values):
            let preview = Array(values.prefix(8))
...
public struct GGUFFile {
    ...
    public let tensorDataStart: Int
    ...
    public func bytes(for tensor: GGUFTensorInfo) throws -> Data {
    ...
    public func decodedFloatPrefix(tensor: GGUFTensorInfo, count: Int) throws -> [Float] {
...
public enum GGMLType {
    public static func name(for raw: UInt32) -> String {
        switch raw {
        case 0: "F32"
        case 1: "F16"
        case 2: "Q4_0"
    ...
    public static func byteCount(type raw: UInt32, elementCount: UInt64) -> Int? {
        switch raw {
...
public struct BinaryReader {
    ...
    public mutating func readUInt8() throws -> UInt8 { try readInteger(UInt8.self) }
    public mutating func readInt8() throws -> Int8 { try readInteger(Int8.self) }
    public mutating func readUInt16() throws -> UInt16 { UInt16(littleEndian: try readInteger(UInt16.self)) }
    public mutating func readInt16() throws -> Int16 { Int16(littleEndian: try readInteger(Int16.self)) }
    public mutating func readUInt32() throws -> UInt32 { UInt32(littleEndian: try readInteger(UInt32.self)) }
    public mutating func readInt32() throws -> Int32 { Int32(littleEndian: try readInteger(Int32.self)) }
    public mutating func readUInt64() throws -> UInt64 { UInt64(littleEndian: try readInteger(UInt64.self)) }
    public mutating func readInt64() throws -> Int64 { Int64(littleEndian: try readInteger(Int64.self)) }
    ...
    public mutating func readGGUFString() throws -> String {
...
public struct GGUFReader {
    ...
    public func read(path: String) throws -> GGUFFile {
        ...
        if case .scalar(let value) = metadata["general.alignment"] {
            alignment = UInt64(value) ?? 32
        } else {
            alignment = 32
        }
    ...
    private func readArray(reader: inout BinaryReader) throws -> GGUFMetadataValue {
        ...
        for _ in 0..<count {
            values.append(try readArrayElement(type: elementType, reader: &reader))
        }
        return .array(type: elementType, count: count, values: values)
    }
